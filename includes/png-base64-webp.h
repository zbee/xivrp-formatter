...
static png_byte *convertPNGToWebP(const png_byte *pngData, int width,
                                  int height, int rowbytes, png_byte color_type,
                                  int *outWebPSize) {
  png_byte *webPData = NULL;
  if (color_type == PNG_COLOR_TYPE_RGB) {
    *outWebPSize =
        WebPEncodeLosslessRGB(pngData, width, height, rowbytes, &webPData);
  } else if (color_type == PNG_COLOR_TYPE_RGBA) {
    *outWebPSize =
        WebPEncodeLosslessRGBA(pngData, width, height, rowbytes, &webPData);
  }
  return webPData;
}
...
char *encodePNGToBase64(const char *imagePath) {
  ...
  int webPSize;
  png_byte *webPData =
      convertPNGToWebP(metaData.data, metaData.width, metaData.height,
                       metaData.rowbytes, metaData.color_type, &webPSize);

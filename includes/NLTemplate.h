...
public:
  virtual void render(std::ostream &output,
                      const Dictionary &dictionary) const = 0;
  virtual ~Fragment();
...
public:
  ...
  void render(std::ostream &output,
              const Private::Dictionary &dictionary) const;
...
private:
  ...
  void load_recursive(const std::string &name,
                      std::vector<Private::Tokenizer> &files,
                      std::vector<Node *> &nodes);
};

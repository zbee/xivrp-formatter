 ...
 * Templated for std::string, std::string_view, std::vector<char> and other char
 * containers.
 */
...
inline RetString encode_with_line_breaks(String s) {
  return insert_linebreaks<RetString, String>(base64_encode(s, false),
                                              line_length);
}
...
inline RetString encode(String s, bool url) {
  return base64_encode<RetString>(
      reinterpret_cast<const unsigned char *>(s.data()), s.size(), url);
}

} // namespace detail

template <typename RetString>
inline RetString base64_encode(const unsigned char *bytes_to_encode,
...
inline RetString decode(const String &encoded_string, bool remove_linebreaks) {
  ...
  while (pos < length_of_string) {
    ...
    //
    ret.push_back(static_cast<typename RetString::value_type>(
        ((pos_of_char(encoded_string.at(pos + 0))) << 2) +
        ((pos_of_char_1 & 0x30) >> 4)));

    if ((pos + 2 < length_of_string) &&
        // Check for data that is not padded with equal signs (which is allowed
        // by RFC 2045)
        encoded_string.at(pos + 2) != '=' &&
        encoded_string.at(pos + 2) != '.') { // accept URL-safe base 64 strings,

...
Tokenizer::Tokenizer(const std::string &text)
    : text_ptr(text.c_str()), len(text.length()), pos(0), peeking(false) {}
...
a:
  if (pos < len) {
    ...
    if (m > 0) {
      ...
      pos += s_endblock_len;
    } else if ((m = match_tag_with_param(s_include, text_ptr + pos,
                                         peek.value)) > 0) {
...
bool Fragment::isBlockNamed(const std::string &) const { return false; }
...
Fragment *Text::copy() const { return new Text(text); }

Property::Property(const std::string &name) : name(name) {}

void Property::render(std::ostream &output,
                      const Dictionary &dictionary) const {
  output << dictionary.find(name);
}

Fragment *Property::copy() const { return new Property(name); }
...
Block::Block(const std::string &name)
    : name(name), enabled(true), resized(false) {}
...
void Block::enable() { enabled = true; }

void Block::disable() { enabled = false; }
...
Node &Block::operator[](size_t index) { return *nodes.at(index); }
...
void Template::load_recursive(const std::string &name,
                              std::vector<Tokenizer> &files,
